...
pygame.display.set_caption("Mini Automat") # Nadpis Okna
...
bigg_font = pygame.font.SysFont(None, 72)
bold_font = pygame.font.SysFont("arial", 48, bold=True)
desc_font = pygame.font.SysFont(None, 48)
...
#                    print("Nemáš dost chipů.")
    ...
    # ==== Menu Design ====
    if game_state == "menu":
        title = title_font.render("MINI AUTOMAT", True, YELLOW)                                                  # Nadpis
        center_y = HEIGHT // 2 - 250                                                                             # Posunutí Nadpisu 
        offset_surface.blit(title, (WIDTH // 2 - title.get_width() // 2, center_y))
    ...
    # ==== Spin Design ====
        ...
        slot_size = 200
        slot_spacing = 20
        total_slot_width = len(slots) * slot_size + (len(slots) - 1) * slot_spacing
        slot_start_x = WIDTH // 2 - total_slot_width // 2
        slot_y = HEIGHT // 2 - slot_size // 2 - 50
...
#        draw_button("Upgrady", button_x, button_y + button_height + button_margin, button_width, button_height, None, surface=offset_surface)
        ...
        for i, symbol in enumerate(slots):
            img = pygame.transform.scale(symbols[symbol], (slot_size, slot_size))
            x = slot_start_x + i * (slot_size + slot_spacing)
            offset_surface.blit(img, (x, slot_y))
...
#        offset_surface.blit(chip_icon_small, (img_x, img_y))
        ...
        offset_surface.blit(text, (WIDTH // 2 - text.get_width() // 2, slot_y - 100))

        # === Skóre (větší a blíž ke slotům) ===
        score_text = big_font.render("Skóre: " + str(total_score), True, YELLOW)
        offset_surface.blit(score_text, (WIDTH // 2 - score_text.get_width() // 2, slot_y + slot_size + 20))
        ...
        offset_surface.blit(spins_text, (WIDTH // 2 - spins_text.get_width() // 2, slot_y + slot_size + 70))
        ...
        if game_state == "show_result":
            ...
            offset_surface.blit(prompt, (WIDTH // 2 - prompt.get_width() // 2, slot_y + slot_size + 120))
    ...
    if flash_timer and pygame.time.get_ticks() - flash_timer < 200:                                 # Jak dlouho to bude v ms (asi?)
        flash_overlay = pygame.Surface((1920, 1080))
