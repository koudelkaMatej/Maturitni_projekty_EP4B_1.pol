...
def ztmav_barvu(hex_color: str, faktor: float) -> str:
    """faktor 0.0–1.0, kde 1 = původní barva, menší = tmavší"""
...
# =========================
okno = turtle.Screen()
okno.title("Hadí hra (hezčí + Skins 1/2/3)")
...
rychlost_ms = 90  # menší = rychlejší
...
def kresli_mrizku(krok=40):
    ...
    for x in range(-MAX_XY, MAX_XY + 1, krok):
        mrizka.goto(x, -MAX_XY); mrizka.pendown(); mrizka.goto(x, MAX_XY); mrizka.penup()
    for y in range(-MAX_XY, MAX_XY + 1, krok):
        mrizka.goto(-MAX_XY, y); mrizka.pendown(); mrizka.goto(MAX_XY, y); mrizka.penup()
...
# Oči
oko_L = turtle.Turtle(); oko_L.hideturtle(); oko_L.speed(0); oko_L.shape("circle")
oko_L.shapesize(0.22, 0.22); oko_L.color("black"); oko_L.penup()
oko_P = turtle.Turtle(); oko_P.hideturtle(); oko_P.speed(0); oko_P.shape("circle")
oko_P.shapesize(0.22, 0.22); oko_P.color("black"); oko_P.penup()
...
def aktualizuj_oblicej_hada():
    if not hra_bezi:
        oko_L.hideturtle(); oko_P.hideturtle(); jazyk.hideturtle()
        return
    ...
    if hlava.direction == "nahoru":
        oko_L.goto(x - 6, y + 6); oko_P.goto(x + 6, y + 6)
        jazyk.hideturtle()
        ...
        if random.random() < 0.22:
            jazyk.showturtle(); jazyk.setheading(180); jazyk.goto(x - 14, y)
        else:
            jazyk.hideturtle()
    elif hlava.direction == "doprava":
        oko_L.goto(x + 6, y + 6); oko_P.goto(x + 6, y - 6)
        if random.random() < 0.22:
            jazyk.showturtle(); jazyk.setheading(0); jazyk.goto(x + 14, y)
        else:
            jazyk.hideturtle()
    else:
        oko_L.goto(x - 6, y + 6); oko_P.goto(x + 6, y + 6)
...
def tick():
    ...
    for i in range(len(telo) - 1, 0, -1):
        ...
        telo_smer[i] = telo_smer[i - 1]  # přenes směr
    if len(telo) > 0:
    ...
    if hlava.distance(jidlo) < 1:
        ...
        novy.speed(0)
        novy.shape("square")
        novy.shapesize(0.70, 1.15)  # výška, šířka 
        novy.color(barva)
        novy.penup()
